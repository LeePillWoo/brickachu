...
`train-test.mjs`는 가까운 순 합류, 실제 이동 경로 추종, 대열 간격, 장애물과 경계, 크기 변화, 합류 동선, 간식 속도 제한, 경로 취소·페이드·삭제를 검사합니다. 기차가 있는 동안 합류 전 동물까지 포식자를 피하지 않는지, 제거 후에는 원래 행동으로 돌아오는지도 확인합니다. 살아난 블록 친구는 기차 대열에서 제외합니다.
