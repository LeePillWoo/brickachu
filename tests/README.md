...
모델에는 같은 Three.js 버전의 `RoundedBoxGeometry`와 `BufferGeometryUtils`를 사용합니다. `models-test.mjs`는 둥근 모서리와 얇은 배·얼굴 패치의 실제 피킹, 회전된 부품 병합 전후의 크기·피킹·공유 자원 정리, 코드에 있는 38종 전체의 지면 착지와 모델당 최대 8개 재질 묶음·5,000개 미만 삼각형을 검사합니다. 기차 밧줄은 무광 회갈색이며, 기존 두께의 절반과 모바일 최소 2 CSS 픽셀을 유지하는지 확인합니다.
